...
contract DeggToken is BEP20('Dragon Egg', 'DEGG') {
...
// DgemBar with Governance.
contract DgemBar is BEP20('DgemBar Token', 'DGEM') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
...
//     // Perform LP token migration from legacy DragonEggFarm to DEGG.
//     // Take the current LP token address and return the new LP token address.
//     // Migrator should have full access to the caller's LP token.
//     // Return the new LP token address.
//     //
//     // XXX Migrator must have allowance access to DragonEggFarm LP tokens.
//     // DEggFarm must mint EXACTLY the same amount of DragonEggFarm LP tokens or
//     // else something bad will happen. Traditional DragonEggFarm does not
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        degg.mint(address(dgem), deggReward);
    ...
    function safeDeggTransfer(address _to, uint256 _amount) internal {
        dgem.safeDeggTransfer(_to, _amount);
    }
